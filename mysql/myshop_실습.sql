...
SELECT O.ORDER_ID, CUSTOMER_ID, E.EMPLOYEE_ID, O.ORDER_DATE, O.TOTAL_DUE
	FROM ORDER_HEADER O, EMPLOYEE E
    WHERE O.EMPLOYEE_ID = E.EMPLOYEE_ID
    AND O.TOTAL_DUE >8500000;
-- Q02) 위에서 작성한 쿼리문을 복사해 붙여 넣은 후 고객이름도 같이 조회되게 수정하세요.
SELECT O.ORDER_ID, C.CUSTOMER_ID, C.CUSTOMER_NAME, E.EMPLOYEE_ID, O.ORDER_DATE, O.TOTAL_DUE
	FROM ORDER_HEADER O, EMPLOYEE E, CUSTOMER C
    WHERE O.EMPLOYEE_ID = E.EMPLOYEE_ID
    AND O.CUSTOMER_ID = C.CUSTOMER_ID
    AND O.TOTAL_DUE >8500000;
-- Q03) Q01 쿼리를 복사해 붙여 넣은 후 직원이름도 같이 조회되게 수정하세요.
SELECT O.ORDER_ID, CUSTOMER_ID, E.EMPLOYEE_ID, E.EMPLOYEE_NAME,O.ORDER_DATE, O.TOTAL_DUE
	FROM ORDER_HEADER O, EMPLOYEE E
    WHERE O.EMPLOYEE_ID = E.EMPLOYEE_ID
    AND O.TOTAL_DUE >8500000;
-- Q04) 위에서 작성한 쿼리문을 복사해 붙여 넣은 후 고객이름, 직원이름도 같이 조회되게 수정하세요.
SELECT O.ORDER_ID, C.CUSTOMER_ID, C.CUSTOMER_NAME, E.EMPLOYEE_ID, E.EMPLOYEE_NAME, O.ORDER_DATE, O.TOTAL_DUE
	FROM ORDER_HEADER O, EMPLOYEE E, CUSTOMER C
    WHERE O.EMPLOYEE_ID = E.EMPLOYEE_ID
    AND O.CUSTOMER_ID = C.CUSTOMER_ID
    AND O.TOTAL_DUE >8500000;
...
SELECT D.ORDER_ID, D.PRODUCT_ID, D.ORDER_QTY, H.SUB_TOTAL, H.TOTAL_DUE
	FROM ORDER_DETAIL D, ORDER_HEADER H
    WHERE D.ORDER_ID = H.ORDER_ID
    AND D.ORDER_QTY >= 30;
-- Q08) 위에서 작성한 쿼리문을 복사해서 붙여 넣은 후 상품이름도 같이 조회되도록 수정하세요.
SELECT D.ORDER_ID, D.PRODUCT_ID, P.PRODUCT_NAME, D.ORDER_QTY, H.SUB_TOTAL, H.TOTAL_DUE
...
SELECT P.SUB_CATEGORY_ID, C.CATEGORY_ID
...
SELECT DISTINCT P.PRODUCT_NAME, E.EMPLOYEE_NAME, H.ORDER_DATE
	FROM EMPLOYEE E, ORDER_HEADER H, PRODUCT P, ORDER_DETAIL D
    WHERE E.EMPLOYEE_ID = H.EMPLOYEE_ID
    AND H.ORDER_ID = D.ORDER_ID 
    AND P.PRODUCT_ID = D.PRODUCT_ID
    AND E.EMPLOYEE_NAME = '다정한'
    AND LEFT(H.ORDER_DATE, 7) BETWEEN '2019-01' AND '2019-12';
    
-- Q12) 청소기를 구입한 고객아이디, 사원번호, 주문번호, 주문일시를 조회하세요.
SELECT DISTINCT E.EMPLOYEE_ID, H.ORDER_DATE, H.ORDER_ID, H.ORDER_DATE, P.PRODUCT_NAME
